...
$$
\begin{aligned}
P(t) &=1-P_0\left(r=0 \right) \&= 1 - \frac{\left(1 + \frac{mt}{k} \right)^{-k} \cdot \Gamma\left(k + 0 \right)}{\Gamma\left(0 + 1 \right)\cdot \Gamma\left(k \right)} \cdot \left(\frac{mt}{mt+k} \right)^0  \&= 1 - \left(1 + \frac{mt}{k} \right)^{-k}
\end{aligned}
$$
</div>
...
$$ 
\begin{aligned}
P_1&=P\left(1 \right) \&= \frac{\left(1 + \frac{0.4125}{0.09899} \right)^{-0.09899} \cdot \Gamma\left(0.09899 + 1 \right)}{\Gamma\left(1 + 1 \right)\cdot \Gamma\left(0.09899 \right)} \& \quad  \quad  \quad \times \left(\frac{0.4125}{0.4125+0.09899} \right)^1 \&= 0.0679
\end{aligned}
$$ 
</div>
...
$$
\begin{aligned}
\text{補正後の平均購入回数}&=\frac{補正後のm}{補正後の浸透率\rm \times 1回当たり平均購入個数} \&=\frac{m'}{\tau' \times 1.1} = \frac{0.5893}{0.1747 \times 1.1}=3.07
\end{aligned}
$$
</div>
となります。
...
$$
\begin{aligned}
P_{0}' &= 0.8253 \P_{1}' &= 0.0700 \P_{2+}' &=0.1047  \\end{aligned}
$$
</div>
...
$$
\begin{aligned}
& \text{2回以上購入者比率} \&=\text{パネル・データの値} \&\quad \quad \times \frac{\text{補正後のモデルによる2回以上購入者比率}}{\text{補正前のモデルによる2回以上購入者比率}} \&=0.5 \times \frac{0.5995}{0.5476}=0.5474
\end{aligned}
$$
</div>
...
$$
\begin{aligned}
& \text{マーケットシェアのパイオニアブランドに対する比率} \&= \left(\text{参入順位}\right)^{-0.49} \times \left(\text{相対的好意度}\right)^{1.11} \times \\ 
...
$$
\begin{aligned}
& \text{予測されるシェア} \&=0.35 \times \left(\text{4}\right)^{-0.49} \times \left(\text{0.9}\right)^{1.11} \left(\text{0.7}\right)^{0.28} \times \left(\text{1}\right)^{0.07} = 0.14285
\end{aligned}
$$
</div>
