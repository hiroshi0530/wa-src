...
これらの指標を用いて、顧客をさまざまなグループに分類し、それぞれに合ったマーケティング戦略を立てます。例えば、最近購入していないが過去に頻繁に購入していた顧客には、再購入を促すような特別なオファー（割引券など）を提供することが考えられます。

RFM分析は、特にリピート購入が重要なビジネスモデルにおいて、顧客の行動をより深く理解するのに非常に有効な方法です。

この分析を通じて、マーケティングのROI（投資利益率）を高め、顧客満足度を向上させることができます。

以下、pandasを利用して、Recency、Frequency、Monetaryのそれをぞれを各ユーザー、各アイテムごとに計算します。

三つのカテゴリごとにDataFrameを作成します。
...
上記で作成した3つのテーブルを結合して、RFM分析をグラフを利用してより簡単に実行できるようにします。
...
![svg](04_nb_files/04_nb_45_1.svg)
    



    
![svg](04_nb_files/04_nb_45_2.svg)
    


#### Frequency VS Recency

例として、FrequencyとRecencyの2次元の散布図を各アイテムごとに図示します。

以下の散布図では、たとえば、右下の最終購入日が遙か昔で、頻度も小さいユーザーを対象にマーケティング施策を検討することが考えられます。
...
![svg](04_nb_files/04_nb_47_0.svg)
    



    
![svg](04_nb_files/04_nb_47_1.svg)
    



    
![svg](04_nb_files/04_nb_47_2.svg)
    


#### 3次元グラフによる可視化

最後に、各アイテムに対して、RFMの3つを3次元のグラフ上にプロットしてみます。
...
![svg](04_nb_files/04_nb_49_0.svg)
    


グラフを見ると、どのデータセットも手前側（最終購入日が遠く、頻度も小さく、金額も少ない）にある程度ユーザーが存在します。

まずは、これらのユーザーに対して再購入を促すための割引やクーポンの提供など何かしらの施策を打ち出すことが考えられます。

また、ItemIDが2の商品は、頻度が大きいが、金額が小さいユーザーのクラスタが存在しているので、頻繁な購入者向けの特別割引や限定商品の提供などの施策が考えられます。

## 結論

ポアソン分布とガンマ分布を利用して、ダミーのデータベースを作成し、そのデータをもとにRFM分析を実行しました。

Recency、Frequency、Monetaryそれぞれに注目し、各ユーザーがどこに位置するかグラフを利用して可視化しました。

各ユーザーがグラフ上のどこに位置するかで、ユーザーに対するマーケティング施策を打ち出すことできます。
